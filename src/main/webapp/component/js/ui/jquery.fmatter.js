...
;(function($) {
	...
	$.fn.fmatter.link = function(cellval, opts,rid) {
		...
		if(!$.fmatter.isEmpty(cellval)) {
			return "<a "+target+" href=\"#\" onclick=\"jQuery('#"+opts.gid+"').jqGrid('showRow','"+rowid+"');\" >" + cellval + "</a>";
		}else {
			return $.fn.fmatter.defaultFormat(cellval,opts);
		}
	};
	...
	$.fn.fmatter.actions = function(cellval,opts, rwd) {
		...
		if(op.editbutton){
			ocl = "onclick=jQuery('#"+opts.gid+"').jqGrid('editRow','"+rowid+"');";
		...
		if(op.delbutton) {
			ocl = "onclick=jQuery('#"+opts.gid+"').jqGrid('delRow','"+rowid+"');";
