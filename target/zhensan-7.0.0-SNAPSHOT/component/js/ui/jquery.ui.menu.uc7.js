...
function createMenu(items){
	...
	var menu = null;
	var menuTarget = items.target.parent();
	var items = items.children;
	if(menuTarget.parent().parent().find('.ui-menu').length>0){
		menuTarget.parent().parent().find('.ui-menu').hide();
	}
	if(menuTarget.children('ul').length>0){
		if (menuTarget.find('ul').is(":visible")) {
			winHide(menuTarget.find('ul'));
			return false;
		}else{
			menuTarget.find('ul').menu().show(0.5,function(){
		...
		menu = $('<ul></ul>').appendTo(menuTarget).show(0.5,function(){
			myLayout.allowOverflow('north');
		});
		$(items).each(function(){
			var that = this;
			var li = $('<li><a href="#" >'+this.value+'</a></li>').appendTo(menu);
...
function winHide(target){
	target.hide(0.5,function(){
		myLayout.resetOverflow('north');
	});
};
