...
//				if(cm.formatter == 'image'){
//					v ="<img src=\"/widgetDemo/themes/ui-lightness/"+cellval+"\" />";
//				}else{
...
//				}
		...
		addJSONData = function(data,t, rcnt, more, adjust) {
			...
			//取记录集
			ts.p.rows = $.jgrid.getAccessor(data,dReader.rows) || {};
			...
			if(f.length>0 && !isNaN(idn)) {
				//填充idn
				if (ts.p.rows && ts.p.rows.length) {
					idn = $.inArray(idn, ts.p.rows);
				}
				idn=f[idn];
			}
			...
			while (i<len) {
				...
				//----------------
//					for (i=0;i<ts.p.colModel.length;i++){
//						alert("____"+ts.p.colModel[i].name);						
//						v = $.jgrid.getAccessor(cur,ts.p.colModel[i].name);
//						alert(v);
...
//					}
		...
		updatepager = function(rn, dnd) {
			...
			var total;
			//计算总记录数
			if(ts.p.records%ts.p.rowNum>0){
				total = Math.floor(ts.p.records/ts.p.rowNum)+1;
			}else{
				total = Math.floor(ts.p.records/ts.p.rowNum);
			}
			...
			if(pgboxes) {
				...
				if(ts.p.pginput===true) {
					//当总记录数小于等于1时，输入框置灰
					if(total>1){
						$('.ui-pg-input',pgboxes).attr("disabled",false);
					}else{
						$('.ui-pg-input',pgboxes).attr("disabled",true);
					}
		...
		populate = function (npage) {
			if(!ts.grid.hDiv.loading) {
				...
				//为组件传输复杂的object做类型转换
				var data;
				if(ts.p.mcontentType=='application/json;charset=UTF-8'){
					data = JSON.stringify(ts.p.postData);
				}else{
					data = ts.p.postData;
				}
				switch(dt)
				...
				case "script":
					$.ajax($.extend({
						...
						type:ts.p.mtype,
						dataType: dt ,
						contentType:ts.p.mcontentType,
						data: $.isFunction(ts.p.serializeGridData)? ts.p.serializeGridData.call(ts,ts.p.postData) : data,
						success:function(data,st) {
							if(dt === "xml") { addXmlData(data,ts.grid.bDiv,rcnt,npage>1,adjust); }
							else { 
								if(data.is_scuess!=null&&data.is_scuess=='false'){
		...
		if(this.p.multiselect) {
			...
			$('#cb_'+$.jgrid.jqID(ts.p.id),this).bind('click',function(){
				if (this.checked) {
					...
					$("[id^=jqg_"+ts.p.id+"_"+"]").attr("checked",false);
					$(ts.rows).each(function(i) {
		...
		$("th",thr).each(function ( j ) {
			...
			firstr += "<td role='gridcell' style='height:0px;width:"+w+"px;"+hdcol+"'>";
			...
			if(cSel === true) {
				if(ts.p.cellEdit === true) {
					...
					if(ts.p.multiselect && ts.p.multiboxonly) {
						if(scb){$(ts).jqGrid("setSelection",ptr[0].id,true);
							...
							$(ts.p.selarrrow).each(function(i,n){
								...
								$("#jqg_"+ts.p.id+"_"+$.jgrid.jqID(n)).prop("checked",false);
							});
							ts.p.selarrrow = [];
					...
					if(e[ts.p.multikey]) {
						...
						scb = $("[id^=jqg_"+ts.p.id+"_"+"]").prop("checked");
						$("[id^=jqg_"+ts.p.id+"_"+"]").prop("checked",!scb);
					}
				}
...
$.jgrid.extend({
		...
		return this.each(function(){
			...
			if(!$t.p.multiselect) {
				...
				if (  ia === -1 ){ 
					...
					stat = true;
					//添加multiboxonly的判断，让checkbox的选中事件和行选中事件可以分离
					if(!$t.p.multiboxonly){
					$("#jqg_"+$t.p.id+"_"+$.jgrid.jqID($t.p.selrow)).prop("checked",stat);
					}
		...
		return this.each(function(){
			...
			if(!t.p.multiselect) {
				...
				$(t.p.selarrrow).each(function(i,n){
					...
					$("#jqg_"+t.p.id+"_"+$.jgrid.jqID(n)).prop("checked",false);
				});
	...
	delRow : function(id){
		this.each(function(){
			var $t=this, pos=-1;
			if ($.isFunction($t.p.onDelRow) ) { ret = $t.p.onDelRow.call($t,id); }
		});
	},
	//添加查看行方法
	showRow : function(id){
		this.each(function(){
			var $t=this, pos=-1;
			if ($.isFunction($t.p.onClickLink) ) { ret = $t.p.onClickLink.call($t,id); }
		});
	},
	//添加删除行方法
	editRow : function(id){
		this.each(function(){
			var $t=this, pos=-1;
			if ($.isFunction($t.p.onEditRow) ) { ret = $t.p.onEditRow.call($t,id); }
		});
	},
